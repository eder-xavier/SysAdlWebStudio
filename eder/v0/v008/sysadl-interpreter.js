        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.endsWith("{")) {
                ...
                if (blockHeader.startsWith("package")) {
                    ...
                    const match = blockHeader.match(/requirement\s+def\s+(\w+)\s*(?:\(([^)]*)\))?/);
                    const name = match[1];
                    currentBlock = "requirement";
                    sysadlModel.requirements[resolveQualifiedName(name, currentPackage)] = { name: name, condition: null, text: null, satisfiedBy: [] };
            ...
            if (line === "}") {
                ...
                if (blockDepth === 0 || currentBlock !== "package") {
                    if (currentBlock === "component" && currentComponent) {
                        ...
                        sysadlModel.requirements[name] = new SysADLRequirement(
                            name.split(".").pop(),
                            name.includes(".") ? name.split(".").slice(0, -1).join(".") + "." : "",
                            sysadlModel.requirements[name].condition,
                            sysadlModel.requirements[name].text,
                            sysadlModel.requirements[name].satisfiedBy
                        );
            ...
            if (line.startsWith("flow")) {
                ...
                const modelName = line.split(" ")[1].replace(";", "");
                log(`Model defined: ${modelName}`);
            }
        }
