    ...
    async send(data, subPortName = null) {
        ...
        if (this.subPorts.size > 0 && subPortName) {
            ...
            if (subPort.bindings.length > 0) {
                await subPort.bindings[0].connector.transmit(data); // Usa o conector do primeiro binding
            } else {
...
async function CheckPresenceToSetTemperature(params = {}) {
    ...
    const presence = params.detected || false;
...
async function main() {
    ...
    await system.addConnector('target', new CTemperatureCN());
    await system.addConnector('average', new CTemperatureCN());
