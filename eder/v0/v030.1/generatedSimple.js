...
class SystemCP extends SysADLComponent {
  ...
  async start() {
    console.log(`Iniciando componente composto `);;
...
class TempMonitorCP extends SysADLComponent {
  ...
  executeTempMonitorAC() {
    console.log(`Executando atividade TempMonitorAC no componente ${this.name}`);;
    const params = {
      s1: this.state['s1'],
      s2: this.state['s2']
    };
    ...
    try {
      validateCalcAverageEQ({ t1: params.s1, t2: params.s2, av: result });
    } catch (e) {
    ...
    if (averagePort) {
      console.log(`Enviando ${result} pela porta average`);;
