...
function parseSysADL(content) {
    ...
    // Extrair tipos (value type, enum, datatype)
    const typeMatches = content.matchAll(/(value\s+type|enum|datatype)\s+def\s+(\w+)(?:\s+extends\s+(\w+))?\s*{([^}]*)}/gs);
    for (const match of typeMatches) {
        const type = { kind: match[1], name: match[2], extends: match[3] || null, content: match[4] };
