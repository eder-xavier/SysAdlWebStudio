...
export class SystemCP extends SysADLComponent {
    ...
    configureBindings() {
        ...
        const temp1 = this.subComponents.get('s1').ports.find(p => p.name === 'temp1');
        const s1 = this.subComponents.get('tempMon').ports.find(p => p.name === 's1');
        if (!temp1 || !s1) {
        ...
        const temp2 = this.subComponents.get('s2').ports.find(p => p.name === 'temp2');
        const s2 = this.subComponents.get('tempMon').ports.find(p => p.name === 's2');
        if (!temp2 || !s2) {
            console.error('Error: One or more ports not found for configuring bindings', {
                temp2: temp2?.name,
                s2: s2?.name
        ...
        const average = this.subComponents.get('tempMon').ports.find(p => p.name === 'average');
        const avg = this.subComponents.get('stdOut').ports.find(p => p.name === 'avg');
        if (!average || !avg) {
            console.error('Error: One or more ports not found for configuring bindings', {
