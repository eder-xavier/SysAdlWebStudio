...
function extractArchitectureData(model, logElement) {
  ...
  // Helper to infer bindings by inspecting port connections
  function inferBindings(model, connector, participantSchema) {
    const bindings = {};
    if (connector && participantSchema) {
      // Assume bind() links ports internally; try to inspect connected ports
      const fromRole = Object.keys(participantSchema).find(role => participantSchema[role].role === 'source');
      const toRole = Object.keys(participantSchema).find(role => participantSchema[role].role === 'target');

      if (fromRole && toRole) {
        // Search for ports in components that match the participant schema
        Object.values(model.components || {}).forEach(comp => {
          Object.entries(comp.ports || {}).forEach(([portName, port]) => {
            if (port.portClass === participantSchema[fromRole].portClass && port.direction === participantSchema[fromRole].direction) {
              bindings[fromRole] = { owner: comp.name, port: { name: portName } };
            }
            if (port.portClass === participantSchema[toRole].portClass && port.direction === participantSchema[toRole].direction) {
              bindings[toRole] = { owner: comp.name, port: { name: portName } };
            }
          });
        });
      }
    ...
    if (comp.connectors && typeof comp.connectors === 'object') {
      Object.entries(comp.connectors).forEach(([connName, conn]) => {
        ...
        if (conn.flowSchema && Array.isArray(conn.flowSchema) && conn.flowSchema.length > 0) {
          ...
          const participantSchema = conn.participantSchema || conn.props?.participantSchema;
          if (participantSchema) {
            fromPort = participantSchema[flow.from];
            toPort = participantSchema[flow.to];

            if (fromPort && toPort) {
              // Infer bindings since connector.bindings is not available
              const inferredBindings = inferBindings(comp, conn, participantSchema);
              const fromBinding = inferredBindings[flow.from] || {};
              const toBinding = inferredBindings[flow.to] || {};

              fromCompName = fromBinding.owner;
              fromPort = { role: fromBinding.port?.name || flow.from };
              toCompName = toBinding.owner;
              toPort = { role: toBinding.port?.name || flow.to };
            }
          }
          ...
          const participantSchema = conn.props.participantSchema;
          if (participantSchema) {
            fromPort = participantSchema[flow.from];
            toPort = participantSchema[flow.to];
