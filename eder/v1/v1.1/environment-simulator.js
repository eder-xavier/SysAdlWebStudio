...
// Function to detect model type and load appropriately (Node.js only)
function loadModel(modelPath) {
  if (typeof require === 'undefined') {
    throw new Error('loadModel is only available in Node.js environment');
  }
...
async function runSimulation() {
  ...
  if (typeof process === 'undefined' || typeof require === 'undefined') {
    throw new Error('runSimulation is only available in Node.js environment. Use run() for browser.');
  }
  ...
  try {
    ...
    if (typeof moduleResult === 'function') {
      ...
      if (model.executables) {
        const execNames = Object.keys(model.executables);
        output += `\nExecuting ${execNames.length} functions:\n`;
