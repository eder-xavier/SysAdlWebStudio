...
function generateJsCode(model) {
    ...
    jsCode += `const { performance } = require('perf_hooks');\n\n`;
    ...
    constructor(name = '${port.name}') {
        ...
        ${isComposite && Array.isArray(port.subPorts) && port.subPorts.length > 0 ? port.subPorts.map(sp => {
            ...
            const subPortDirection = model.ports.find(p => p.name === subPortType)?.flows[0]?.direction || 'inout';
    ...
    async start() {
        ...
        // Boundary components will be driven by external input in main
    }
}\n\n`;
    ...
    model.components.forEach(comp => {
        ...
        // Gera os métodos para as atividades alocadas como uma string
