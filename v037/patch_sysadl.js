...
function patchSysADL(generatedCode, model) {
  try {
    ...
    model.activities = Array.isArray(model.activities)
      ? model.activities.map(act => ({ ...act, actions: Array.isArray(act.actions) ? act.actions : [] }))
      : [];
    ...
    const newCode = [...codeLines]; // <-- bug fixed
    ...
    if (connectorSectionIndex !== -1) {
      ...
      model.connectors.forEach(conn => {
        ...
        // Do NOT export here; keep consistency with the rest of the file
        newCode.push(`class ${connName} extends SysADLConnector {`);
