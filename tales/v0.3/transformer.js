...
function extractExecutableParams(body) {
  ...
  lines.push('');
  lines.push('  // Auto-assign activity references after model setup');
  lines.push('  assignActivityReferences() {');
  lines.push('    this.injectModelReference(); // Ensure model references are set');
  
  for (const a of activitiesToRegister) {
    const comp = a.descriptor && a.descriptor.component;
    if (comp) {
      lines.push(`    // Assign activity "${a.activityName}" to instances of type "${comp}"`);
      lines.push(`    this.walkComponents(c => {`);
      lines.push(`      if (c.props && c.props.sysadlDefinition === "${comp}") {`);
      lines.push(`        c.activityName = "${a.activityName}";`);
      lines.push(`      }`);
      lines.push(`    });`);
      lines.push(`    this.walkConnectors(c => {`);
      lines.push(`      if (c.constructor.name.includes("${comp}")) {`);
      lines.push(`        c.activityName = "${a.activityName}";`);
      lines.push(`      }`);
      lines.push(`    });`);
    }
