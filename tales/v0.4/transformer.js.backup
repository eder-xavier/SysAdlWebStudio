...
function generateEnvironmentModule(modelName, environmentElements, traditionalElements, ast, embeddedTypes, packageMap, inputFileName = null) {
  ...
  lines.push(`const { EnvironmentDefinition, EnvironmentConfiguration, Entity, Event, events, eventClasses, Scene, Scenario, ScenarioExecution, EventsDefinitions, SceneDefinitions, ScenarioDefinitions, Connection } = require('../sysadl-framework/SysADLBase');`);
  ...
  // 6. Generate Scene classes (SCN_MoveAGV1toA, etc.)
  for (const { element } of sceneDefinitions) {
    ...
    for (const [sceneName, sceneDef] of Object.entries(scenes)) {
      ...
      if (sceneDef.preConditions && sceneDef.preConditions.length > 0) {
        lines.push(`    this.preConditions = ${JSON.stringify(sceneDef.preConditions)};`);
      } else {
        lines.push(`    this.preConditions = [];`);
      }
      
      if (sceneDef.postConditions && sceneDef.postConditions.length > 0) {
        lines.push(`    this.postConditions = ${JSON.stringify(sceneDef.postConditions)};`);
      } else {
        lines.push(`    this.postConditions = [];`);
      }
  ...
  // 7. Generate SceneDefinitions classes (MyScenes)
  ...
  // 8. Generate Scenario classes (Scenario1)
  for (const { element } of scenarioDefinitions) {
    ...
    for (const [scenarioName, scenarioDef] of Object.entries(scenarios)) {
      ...
      lines.push(`    this.sceneDefs = ${JSON.stringify(scenarioDef.sceneDefs || {})};`);
      lines.push(`  }`);
      lines.push(``);
      lines.push(`  // Execute scenario with all scenes`);
      lines.push(`  async execute(context) {`);
      lines.push(`    if (context.sysadlBase && context.sysadlBase.logger) context.sysadlBase.logger.log('🚀 Starting scenario: ${scenarioName}');`);
      lines.push(`    const results = [];`);
      lines.push(`    for (const sceneName of this.scenes) {`);
      lines.push(`      const scene = this.sceneDefs[sceneName];`);
      lines.push(`      if (scene) {`);
      lines.push(`        const result = await scene.execute(context);`);
      lines.push(`        results.push({ scene: sceneName, result });`);
      lines.push(`      }`);
      lines.push(`    }`);
      lines.push(`    if (context.sysadlBase && context.sysadlBase.logger) context.sysadlBase.logger.log('✅ Scenario completed: ${scenarioName}');`);
  ...
  // 9. Generate ScenarioDefinitions classes (MyScenarios)
  ...
  // 10. Generate ScenarioExecution classes
  for (const { element, className } of scenarioExecutions) {
    const executionName = element.defs || element.name || 'UnnamedExecution';
  ...
  // 4. Generate Event classes (SupervisoryEvents, AGV1Events, AGV2Events, etc.) 
  ...
  // 5. Generate EventsDefinitions classes (MyEvents)
