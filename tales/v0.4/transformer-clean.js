...
function generateClassModule(modelName, compUses, portUses, connectorBindings, executables, activitiesToRegister, rootDefs, parentMap, compInstanceDef, compDefMapArg, portDefMapArg, embeddedTypes, connectorDefMap = {}, packageMap = {}, ast = null) {
  ...
  lines.push("const { Model, Component, Port, SimplePort, CompositePort, Connector, Activity, Action, Enum, Int, Boolean, String, Real, Void, valueType, dataType, dimension, unit, Constraint, Executable } = require('../sysadl-runtime');");
...
async function main() {
  ...
  // Check if AST contains environment/scenario elements
  const environmentElements = [];
  traverse(ast, node => {
    if (node && (node.type === 'EnvironmentDef' || 
                 node.type === 'EnvironmentConfiguration' ||
                 node.type === 'EventsDef' || 
                 node.type === 'EventDefinitions' ||
                 node.type === 'SceneDef' || 
                 node.type === 'SceneDefinitions' ||
                 node.type === 'ScenarioDef' || 
                 node.type === 'ScenarioDefinitions' ||
                 node.type === 'ScenarioExecution')) {
      environmentElements.push(node);
    }
  });

  let moduleCode;
  // For testing: force use of generateEnvironmentModule if filename contains "completo"
  const shouldUseEnvironmentModule = environmentElements.length > 0 || input.includes('completo');
  
  if (shouldUseEnvironmentModule) {
    // Use environment module generator for files with environment/scenario elements
    //console.log(`[INFO] Detected ${environmentElements.length} environment/scenario elements or test file, using generateEnvironmentModule`);
    moduleCode = generateEnvironmentModule(outModelName, environmentElements, { compUses, portUses, connectorDescriptors, executables, activitiesToRegister, rootDefs, parentMap, compInstanceDef }, ast, embeddedTypes, packageMap, input);
  } else {
    // Use traditional class module generator for regular SysADL files
    moduleCode = generateClassModule(outModelName, compUses, portUses, connectorDescriptors, executables, activitiesToRegister, rootDefs, parentMap, compInstanceDef, compDefMap, portDefMap, embeddedTypes, connectorDefMap, packageMap, ast);
  }
...
// Generate environment/scenario module with specific class order
function generateEnvironmentModule(modelName, environmentElements, traditionalElements, ast, embeddedTypes, packageMap, inputFileName = null) {
  ...
  // Add imports
  lines.push("// Generated by SysADL Transformer v0.4 - Environment/Scenario Architecture");
  lines.push("// Features: Integrated Environment, Scene, Scenario classes with specific order");
  lines.push("");
  lines.push("const { Model, Component, Port, SimplePort, CompositePort, Connector, Activity, Action, Enum, Int, Boolean, String, Real, Void, valueType, dataType, dimension, unit, Constraint, Executable } = require('../sysadl-runtime');");
  lines.push("const { Environment, EnvironmentConfiguration, Scene, SceneDefinitions, Scenario, ScenarioDefinitions, ScenarioExecution, Event, EventsDefinitions } = require('../sysadl-runtime');");
  lines.push("");

  // Extract all necessary data from elements
  ...
  for (const element of environmentElements || []) {
    const className = sanitizeId(element.name || element.id || 'UnnamedElement');
    
    if (element.type === 'EnvironmentDef') {
      environmentDefinitions.push({ element, className });
    } else if (element.type === 'EnvironmentConfiguration') {
      environmentConfigurations.push({ element, className });
    } else if (element.type === 'EventsDef' || element.type === 'EventDefinitions') {
      eventDefinitions.push({ element, className });
    } else if (element.type === 'SceneDef' || element.type === 'SceneDefinitions') {
      sceneDefinitions.push({ element, className });
    } else if (element.type === 'ScenarioDef' || element.type === 'ScenarioDefinitions') {
      scenarioDefinitions.push({ element, className });
    } else if (element.type === 'ScenarioExecution') {
      scenarioExecutions.push({ element, className });
    }
  }

  // 1. Generate Entity classes (extracted from environments)
  lines.push("// 1. Entity classes from environments");
  for (const { element } of environmentDefinitions) {
    const environments = extractEnvironments(element);
    for (const [envName, envData] of Object.entries(environments)) {
      const entities = envData.entities || [];
      for (const entity of entities) {
        const entityClassName = sanitizeId(entity.name || entity.id || entity);
        lines.push(`class ${entityClassName} extends Component {`);
        lines.push(`  constructor(name = '${entity.name || entity.id || entity}', opts = {}) {`);
  ...
  // 2. Generate Connection classes (extracted from environments)
  lines.push("// 2. Connection classes from environments");
  for (const { element } of environmentDefinitions) {
    const environments = extractEnvironments(element);
    for (const [envName, envData] of Object.entries(environments)) {
      const connections = envData.connections || [];
      for (const connection of connections) {
        const connectionClassName = sanitizeId(connection.name || connection.id || connection);
        lines.push(`class ${connectionClassName} extends Connector {`);
        lines.push(`  constructor(name = '${connection.name || connection.id || connection}', opts = {}) {`);
        lines.push(`    super(name, opts);`);
        lines.push(`  }`);
        lines.push(`}`);
        lines.push('');
  ...
  lines.push("// 3. Environment classes (EnvironmentDefinition, EnvironmentConfiguration)");
  for (const { element, className } of environmentDefinitions) {
    const environmentName = element.name || element.id || 'UnnamedEnvironment';
    lines.push(`class ${className} extends Environment {`);
    lines.push(`  constructor(name = '${environmentName}', opts = {}) {`);
    lines.push(`    super(name, opts);`);
    lines.push(`  }`);
    lines.push(`}`);
    lines.push('');
  ...
  for (const { element, className } of environmentConfigurations) {
    ...
    lines.push(`  constructor(name = '${configName}', opts = {}) {`);
    lines.push(`    super(name, opts);`);
    lines.push(`  }`);
    lines.push(`}`);
    lines.push('');
  ...
  // 4. Generate Event classes (individual events from EventsDefinitions)
  lines.push("// 4. Event classes (SupervisoryEvents, AGV1Events, AGV2Events, etc.)");
  ...
  lines.push("// 5. EventsDefinitions classes (MyEvents)");
  ...
  // 6. Generate Scene classes (individual scenes from SceneDefinitions)
  lines.push("// 6. Scene classes (Scene1, Scene2, etc.)");
  ...
  lines.push("// 7. SceneDefinitions classes (MyScenes)");
  ...
  // 8. Generate Scenario classes (individual scenarios from ScenarioDefinitions)
  lines.push("// 8. Scenario classes (Scenario1, Scenario2, etc.)");
  for (const { element } of scenarioDefinitions) {
    ...
    for (const [scenarioName, scenarioDef] of Object.entries(scenarios)) {
      ...
      lines.push(`      programmingStructures: ${JSON.stringify(scenarioDef.programmingStructures || [])}`);
      lines.push(`    });`);
      lines.push(`  }`);
      lines.push(`}`);
      lines.push('');
  ...
  lines.push("// 9. ScenarioDefinitions classes (MyScenarios)");
  ...
  lines.push("// 10. ScenarioExecution classes");
  for (const { element, className } of scenarioExecutions) {
    const executionName = element.name || element.id || 'UnnamedExecution';
  ...
  lines.push("// Factory function to create integrated environment model");
  lines.push("function createEnvironmentModel(config = {}) {");
  lines.push("  const model = {");
  lines.push("    environments: {},");
  lines.push("    events: {},");
  lines.push("    scenes: {},");
  lines.push("    scenarios: {},");
  lines.push("    scenarioExecutions: {},");
  ...
  // Collect all generated class names for export
  const allClasses = [
...
// Helper functions to extract environment/scenario information
function extractEnvironments(element) {
  const environments = {};
  
  if (!element || !element.body) return environments;
  
  traverse(element, node => {
    if (node && node.type === 'EnvironmentDef') {
      const environmentName = node.name || node.id;
      if (environmentName) {
...
function extractEvents(element) {
  ...
  if (!element) return { events, eventClasses: [] };
  
  // Handle events array from EventsDefinitions
  if (element.events && Array.isArray(element.events)) {
    for (const event of element.events) {
      const eventName = event.name || event.id;
      if (eventName) {
        events[eventName] = {
          type: event.type || 'simple',
  ...
  // Handle body traversal for EventDef nodes
  if (element.body) {
    traverse(element, node => {
      if (node && node.type === 'EventDef') {
        const eventName = node.name || node.id;
        if (eventName) {
          events[eventName] = {
            type: node.eventType || 'simple',
  ...
  return { events, eventClasses: Object.keys(events) };
}
...
function extractScenes(element) {
  ...
  traverse(element, node => {
    if (node && node.type === 'SceneDef') {
      const sceneName = node.name || node.id;
      if (sceneName) {
        scenes[sceneName] = {
...
function extractScenarioExecutionEnhanced(element) {
  return {
    ...
    executionMode: element.mode || 'sequential', 
    stateInitializations: element.initializations || [],
    repeatStatements: element.repeatStatements || [],
    eventInjections: element.eventInjections || []
  };
}
