...
function generateEnvironmentModule(modelName, environmentElements, traditionalElements, ast, embeddedTypes, packageMap, inputFileName = null) {
  ...
  lines.push(`const { EnvironmentDefinition, EnvironmentConfiguration, Entity, Event, Scene, Scenario, ScenarioExecution, EventsDefinitions, SceneDefinitions, ScenarioDefinitions, Connection } = require('../SysADLBase');`);
  ...
  // Generate ScenarioExecution classes
  for (const { element, className } of scenarioExecutions) {
    ...
    lines.push(`// Scenario Execution: ${executionName}`);
    ...
    lines.push(`      executionMode: '${extractExecutionMode(element)}'`);
    lines.push(`    });`);
    lines.push(`  }`);
    lines.push(`}`);
    lines.push('');
