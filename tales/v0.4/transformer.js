...
function generateEnvironmentModule(modelName, environmentElements, traditionalElements, ast, embeddedTypes, packageMap, inputFileName = null) {
  ...
  lines.push(`// Auto-generated by SysADL Transformer`);
    ...
    for (const [typeName, typeDef] of Object.entries(entityTypes)) {
      ...
      if (typeDef.properties && Object.keys(typeDef.properties).length > 0) {
        ...
        for (const [propName, propType] of Object.entries(typeDef.properties)) {
          lines.push(`    this.properties.${propName} = null; // Type: ${propType}`);
        }
        lines.push(`    `);
      }
  ...
  for (const { element, className } of environmentDefinitions) {
    ...
    // Generate entities array with actual Entity class references
    lines.push(`    // Available entity types (classes)`);
    const entityClassNames = Object.keys(entityTypes).map(name => sanitizeId(name));
    lines.push(`    this.entities = [${entityClassNames.join(', ')}];`);
    lines.push(`    `);
    
    // Generate connections array  
    const connectionClassNames = Object.keys(connections).map(name => sanitizeId(name));
    lines.push(`    // Available connection types`);
    lines.push(`    this.connections = [${connectionClassNames.join(', ')}];`);
    lines.push(`  }`);
    lines.push(`  `);
    
    // Generate createEntity factory method with validation
    lines.push(`  createEntity(typeName, options = {}) {`);
    lines.push(`    // Validation: check if typeName is declared in entities array`);
    lines.push(`    const EntityClass = this.entities.find(cls => cls.name === typeName);`);
    lines.push(`    if (!EntityClass) {`);
    lines.push(`      throw new Error(\`Entity type '\${typeName}' is not declared in this environment definition. Available types: \${this.entities.map(cls => cls.name).join(', ')}\`);`);
    lines.push(`    }`);
    lines.push(`    `);
    lines.push(`    // Create instance with unique name if not provided`);
    lines.push(`    const instanceName = options.name || \`\${typeName.toLowerCase()}_\${Date.now()}\`;`);
    lines.push(`    const instance = new EntityClass(instanceName, options);`);
    lines.push(`    `);
    lines.push(`    // Set properties if provided`);
    lines.push(`    if (options.properties) {`);
    lines.push(`      Object.assign(instance.properties, options.properties);`);
    lines.push(`    }`);
    lines.push(`    `);
  ...
  for (const { element, className } of environmentConfigurations) {
    ...
    lines.push(`  constructor() {`);
    lines.push(`    super(new ${targetName}());`);
    lines.push(`    `);
    ...
    // Add createEntity method for compatibility
    lines.push(`  createEntity(typeName, options = {}) {`);
    lines.push(`    // Map to environment definition entities`);
    lines.push(`    const EntityMap = {`);
    lines.push(`      'Station': Station,`);
    lines.push(`      'PartX': PartX,`);
    lines.push(`      'Lane': Lane,`);
    lines.push(`      'Supervisory': Supervisory,`);
    lines.push(`      'Vehicle': Vehicle`);
    lines.push(`    };`);
    lines.push(`    `);
    lines.push(`    const EntityClass = EntityMap[typeName];`);
    lines.push(`    if (!EntityClass) {`);
    lines.push(`      throw new Error(\`Entity type '\${typeName}' is not available. Available types: \${Object.keys(EntityMap).join(', ')}\`);`);
    lines.push(`    }`);
    lines.push(`    `);
    lines.push(`    return new EntityClass(options);`);
    lines.push(`  }`);
