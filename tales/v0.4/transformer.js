...
function generateEnvironmentModule(modelName, environmentElements, traditionalElements, ast, embeddedTypes, packageMap, inputFileName = null) {
  ...
  lines.push(`const { EnvironmentDefinition, EnvironmentConfiguration, Entity, Event, Scene, Scenario, ScenarioExecution } = require('../SysADLBase');`);
  ...
  for (const element of environmentElements) {
    ...
    const elementName = element.name || (element.id && element.id.name) || element.id || 'UnnamedElement';
    const className = sanitizeId(elementName);
  ...
  // Generate EnvironmentDefinition classes
  for (const { element, className } of environmentDefinitions) {
    ...
    lines.push(`      ...opts,`);
    lines.push(`      entityTypes: ${JSON.stringify(extractEntityTypes(element))},`);
    lines.push(`      eventTypes: ${JSON.stringify(extractEventTypes(element))},`);
    lines.push(`      properties: ${JSON.stringify(extractProperties(element))}`);
    lines.push(`    });`);
    lines.push(`  }`);
    lines.push(`}`);
    lines.push('');
  ...
  // Generate Entity classes from environment definitions
  for (const { element } of environmentDefinitions) {
    ...
    for (const [typeName, typeDef] of Object.entries(entityTypes)) {
      ...
      lines.push(`      properties: ${JSON.stringify(typeDef.properties || {})}`);
      lines.push(`    });`);
      lines.push(`  }`);
      lines.push(`}`);
      lines.push('');
  ...
  for (const { element, className } of scenarioExecutions) {
    lines.push(`// Scenario Execution: ${element.name}`);
  ...
  const allClasses = [
    ...
    ...eventDefinitions.flatMap(d => Object.keys(extractEvents(d.element)).map(sanitizeId)),
    ...sceneDefinitions.flatMap(d => Object.keys(extractScenes(d.element)).map(sanitizeId)),
    ...scenarioDefinitions.flatMap(d => Object.keys(extractScenarios(d.element)).map(sanitizeId)),
...
function extractEntityTypes(element) {
  ...
  if (!element || !element.body) return entityTypes;
  
  // Look for Entity def nodes
  traverse(element, node => {
    if (node && node.type === 'EntityDef') {
      const entityName = node.name || (node.id && node.id.name) || node.id;
      if (entityName) {
        const properties = {};
        const roles = {};
