const { Model, Component, Port, SimplePort, CompositePort, Connector, Activity, Action, createExecutableFromExpression, Enum, Int, Boolean, String: SysADLString, Real, Void, valueType, dataType, dimension, unit } = require('../v0.3/SysADLBase');
...
class SysADLModel extends Model {
  constructor(){
    ...
    this.SystemCP.s1.addPort(new PT_FTempOPT("current", "in", { owner: "s1" }));
    this.SystemCP.s2.addPort(new PT_FTempOPT("current", "in", { owner: "s2" }));
    this.SystemCP.tempMon.addPort(new PT_CTempIPT("s1", "in", { owner: "tempMon" }));
    this.SystemCP.tempMon.addPort(new PT_CTempIPT("s2", "in", { owner: "tempMon" }));
    this.SystemCP.tempMon.addPort(new PT_CTempOPT("average", "out", { owner: "tempMon" }));
    ...
    this.addExecutableSafe("SysADLModel.wfn4", "executable FarToCelEX to FarToCelAN", []);
    this.addExecutableSafe("SysADLModel.z3xn", "executable CalcAverageEX to TempMonitorAN", []);
